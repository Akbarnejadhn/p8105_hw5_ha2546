...
First, I chose a sample of 20 from these 150 obserations and eplaced them with NA. I also converted the species factor variable to character. 
...
In this part, I wrote the function _replace_NA_fnc_ that does two things: 1/ It replaces NA's in character vectors with the word _Virginica_, and 2/ It fills NA's in numeric vectors with the mean of non-NA's. This function also has a check poinr and breaks if the vector is not numeric or character. Then I mapped the function to _iris datset_.
...
First, we want to import these files' information and save them in a data frame.
```{r tibble_data}
...
It can be be observed that by increasing the effect size (the true value of beta-1), the power of the test increases. This observation aligns with what we expect since when going away from the null hypothesis (beta-1 = 0), it is more probable that we can notice the difference between the value and the null, get a lower p-value, and consequently reject the null correctly when the null hypothesis is false.

On the next step, I want to see the relationship between average estimate of beta-1 and the true value of beta-1. I would like to observe how this relationship changes when considering the whole sample, and when considering only the ones in which the null hypothesis has been rejected.
...
We previousely noted that by increasing the effect size, the power increases. This means that a higher proportion of sample get rejected when they have to be rejected. This results in decrease in the proportion of those cases in the overall sample in which the null has not been rejected. This is the reason that in this plot, we observe the difference between two lines decreases as the effect size increases; because there are fewer unrejected cases left in the whole samples. The same reason justifys why we observe the highest diffetence in beta_1 = 1 case, since the effect size is small and halph of the cases which should be rejected stay unrejected. This trend is observed because instead of taking random samples, we are choosing the ones with p_value < 0.05, so the result we get is not unbiased anymore.
