...
sim_results %>% 
  ...
  labs(
    x= "Beta_1 Values",
    y = "Rejected Null Proportion (p_val < 0.05)",
    title = NULL,
    caption = NULL
  ) 
```
...
Describe the association between effect size and
power.
...
comparison_plot = ggplot() +                      # using above dataframes to overlay two plots showing the trends
  ...
  labs(
    title = "    beta_1 estimates in different beta_1 values
    in all sample (yellow line) vs. samples with rejected null (purple line)",
    x = "beta_1 Values",
